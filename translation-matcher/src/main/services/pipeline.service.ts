...
export class PipelineService {
  ...
  private async indexCorpus(corpusConfig: any, type: 'A' | 'B', aiConfig: any) {
    ...
    for (const file of files) {
      ...
      const prompt = type === 'B' 
        ? PROMPTS.INDEX_B(text.substring(0, 15000)) // Truncate for token limits
  ...
  private async findCandidates(aiConfig: any) {
    ...
    for (const docA of docsA as any[]) {
      ...
      // Naive batching: send all B summaries (in real app, split into batches)
  ...
  private async verifyMatches(aiConfig: any) {
    ...
    for (const cand of candidates as any[]) {
      ...
      // In a real app, we'd extract specific pages based on earlier hints.
      // Here we send start chunks.
  ...
  private log(message: string) {
    ...
    if (win) win.webContents.send('pipeline:log', message);
  }
}
...
const PROMPTS = {
  ...
  CANDIDATE_SELECTION: (docA: any, docsB: any[]) => `
    ...
    Identify plausible source texts based on topics, proper names, and structural similarities.
  `,
  ...
  CITATION: (docBIndex: any) => `
    Generate a Chicago-style bibliography entry for this document.
